<template>
  <div id="shopcar">
    <div id="content">
      <div class="header">
        <img src="./../../common/img/20201003110432.png" class="header_logo" />
        <span>购物车</span>
      </div>
      ...
      <div class="foot_total">
        ...
        <div class="btn-area">
          <a
            ...
            @click="btn(totalPrice, totalAmount)"
            >结&nbsp;算</a
          >
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    let count = this.goods.buy_count
    // 请求商品数据
    this.$store.dispatch("reqCartsGoods", { user_id }, "delCartsGoods");
  },
  methods: {
    // 1.更新单个商品数量
    ...
    async clickTrash(goods) {
      ...
      await MessageBox.confirm("您确定删除该商品吗？").then((action) => {
        if (action === "confirm") {
          let result = deleteGoods(goods.goods_id);
          console.log("删除商品成功");
    ...
    btn( totalPrice,totalAmount) {
      console.log(totalPrice);
      this.$router.push({name:'order',params: {totalPrice: totalPrice,totalAmount:totalAmount}});
    },
  },
};
</script>
