...
router.post('/api/delete_goods', (req, res) => { 
    ...
    let sql_str = "DELETE FROM cart WHERE goods_id = '"+ goods_id +"'";
                conn.query(sql_str, (error, results, fields) => {
                    if (error) {
                        res.json({
                            err_code: 0,
...
router.post('/api/delete_user', (req, res) => {
    ...
    let sqlStr = "DELETE FROM user_info WHERE id =" + id;
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({
...
router.get('/api/address', (req, res) => {
    ...
    let sqlStr = 'SELECT address_id, user_name, user_phone, user_address FROM address';
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({
...
// 拼接地址信息
router.get('/api/address_add', (req, res) => {

    let sqlStr = 'select concat(user_name,\'   \',user_phone,\'   \' ,user_address,\'   \',address_id) AS msg from address';
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({
...
// 删除地址信息
router.post('/api/delete_address', (req, res) => {
    // 获取数据
    const address_id = req.body.address_id;
    console.log(address_id);
    let sqlStr = "DELETE FROM address WHERE address_id = " + address_id;
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({
...
// 新增地址
router.post('/api/add_address', (req, res) => {
    let address_id = req.body.address_id;
    let user_name = req.body.user_name;
    let user_phone = req.body.user_phone;
    let user_address = req.body.user_address;
    let addAddress = 'INSERT INTO address(address_id,user_name,user_phone,user_address) VALUES (?,?,?,?)';
    let userAddSql_Params = [address_id,user_name,user_phone,user_address];
    conn.query(addAddress, userAddSql_Params, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({
                err_code: 0,
